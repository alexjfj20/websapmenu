...
const SYNC_CONFIG = {
  ...
  maxRetries: 2,
  platoUrl: `${API_URL}/sync/platos`, // Esta ruta se dirige al nuevo controlador en syncRoutes.js
  emergencyPlatoUrl: `${API_URL}/platos`, // Actualizado para usar respaldo en app.js
...
// Función para verificar el estado de sincronización con el servidor
const checkSyncStatus = async () => {
  console.log('🔄 Verificando estado de sincronización con el servidor...');
  
  // --- Comentado ---
  // La ruta /api/sync/status no existe en el backend, causando 404.
  // Se comenta esta llamada hasta que se implemente el endpoint en el backend si es necesario.
  /*
  try {
    ...
    if (response.ok) {
      ...
      console.error('❌ Error al verificar estado de sincronización:', errorText);
      return { status: 'error', message: 'No se pudo obtener el estado' };
    }
  } catch (error) {
    console.error('❌ Error de red al verificar estado de sincronización:', error);
    return { status: 'error', message: 'Error de red' };
  }
  */
 // --- Fin Comentado ---

 // Devolver un estado predeterminado o indicar que la función está deshabilitada
 console.warn('⚠️ La verificación de /api/sync/status está deshabilitada temporalmente.');
 return { status: 'unknown', message: 'Verificación deshabilitada' }; 
};
...
const initSyncService = () => {
  ...
  Promise.all([
    ...
    // checkSyncStatus() // Comentado porque la ruta no existe
